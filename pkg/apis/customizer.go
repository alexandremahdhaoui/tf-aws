...
func Customize(source, custom *TerraformModuleDefinition) *TerraformModuleDefinition {
	sf, ef := logger.Trace("customize", "", source.Metadata.Name, string(source.Kind), source.ApiVersion)
	defer sf()
	switch {
...
func customizeTerraformConfig(source, custom *TerraformConfig) *TerraformConfig {
	sf, _ := logger.Trace("customize", "", "TerraformConfig")
	defer sf()
	if custom == nil {
		return source
	}
...
func customizeMetadata(source, custom *Metadata) *Metadata {
	sf, _ := logger.Trace("customize", "", "Metadata")
	defer sf()
	return &Metadata{
		Name:        source.Name,
...
func customizeSpec(source, custom *Spec) *Spec {
	sf, _ := logger.Trace("customize", "", "Spec")
	defer sf()
	if custom == nil {
		return source
	}
...
func customizeArgumentOrAttribute(source, custom ArgumentOrAttribute) ArgumentOrAttribute {
	sf, _ := logger.Trace("customize", "", "ArgumentOrAttribute")
	defer sf()
...
func customizeLifecycle(source, custom *Lifecycle) *Lifecycle {
	sf, _ := logger.Trace("customize", "", "Lifecycle")
	defer sf()
	if custom == nil {
		return source
	}
...
func customizeTerraformProvider(source, custom *TerraformProvider) *TerraformProvider {
	sf, _ := logger.Trace("customize", "", "TerraformProvider")
	defer sf()
...
func customizeDefaultTags(source, custom *DefaultTags) *DefaultTags {
	sf, _ := logger.Trace("customize", "", "DefaultTags")
	defer sf()
	if custom == nil {
		return source
	}
