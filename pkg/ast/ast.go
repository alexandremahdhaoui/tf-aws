...
func FromHtmlNode(node *visitor.Node) token.Tree {
	sf, _ := logger.Debug("convert", "`htmlVisitor.Node` to", "token.Tree")
	defer sf()
	tt := dfs(node)
	return tt
}
...
func cleanToken(tkn token.Token) token.Token {
	sf, _ := logger.Debug("clean", "", "token.Token")
	defer sf()
...
func Flatten(tt token.Tree) []token.Token {
	sf, _ := logger.Debug("flatten", "", "token.Tree")
	defer sf()
