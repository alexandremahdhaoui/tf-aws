...
variable "domain_controllers" {
  description = "(Optional) List of IPv4 addresses, NetBIOS names, or host names of your domain server.\nIf you need to specify the port number include it after the colon (“:”). For example, mydc.mydomain.com:389.In addition to all arguments above, the following attributes are exported:"
  type        = string
  default     = ""
}
...
variable "organizational_unit" {
  description = "(Optional) The organizational unit (OU) is a container in an Active Directory that can hold users, groups,\ncomputers, and other OUs and this parameter specifies the OU that the gateway will join within the AD domain."
  type        = string
  default     = ""
}
