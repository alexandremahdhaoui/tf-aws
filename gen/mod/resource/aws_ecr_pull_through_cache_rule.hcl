resource "aws_ecr_pull_through_cache_rule" "aws_ecr_pull_through_cache_rule" {
...
output "registry_id" {
  ...
  value       = aws_ecr_pull_through_cache_rule.aws_ecr_pull_through_cache_rule.registry_id
}
output "provider_region" {
  description = "Region where the provider should be executed."
  type        = string
...
provider "aws" {
  ...
  default_tags {
    tags {
      ...
      "kind/name"                   = "aws_ecr_pull_through_cache_rule"
      "kind/version"                = "v0.1.0"
    }
  }
}
