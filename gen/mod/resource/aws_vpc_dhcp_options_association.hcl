...
variable "Removing the DHCP Options Association automatically sets AWS's " {
  description = "default DHCP Options Set to the VPC.In addition to all arguments above, the following attributes are exported:"
  type        = string
}
variable "You can only associate one DHCP Options Set to a given VPC ID." {
  description = ""
  type        = string
}
...
provider "aws" {
  ...
  default_tags {
    tags {
      ...
      "kind/name"                   = "aws_vpc_dhcp_options_association"
      "kind/version"                = "v0.1.0"
    }
  }
}
