resource "aws_cloudfront_monitoring_subscription" "aws_cloudfront_monitoring_subscription" {
  distribution_id                      = var.distribution_id
  monitoring_subscription              = var.monitoring_subscription
  realtime_metrics_subscription_config = var.realtime_metrics_subscription_config
...
output "monitoring_subscription" {
  description = "(Required) A monitoring subscription. This structure contains information about whether additional CloudWatch metrics are enabled for a given CloudFront distribution.monitoring_subscription"
  value       = aws_cloudfront_monitoring_subscription.aws_cloudfront_monitoring_subscription.monitoring_subscription
}
output "provider_region" {
  description = "Region where the provider should be executed."
  type        = string
...
output "id" {
  ...
  value       = aws_cloudfront_monitoring_subscription.aws_cloudfront_monitoring_subscription.id
}
output "provider_region" {
  description = "Region where the provider should be executed."
  type        = string
...
provider "aws" {
  ...
  default_tags {
    tags {
      ...
      "kind/name"                   = "aws_cloudfront_monitoring_subscription"
      "kind/version"                = "v0.1.0"
    }
  }
}
