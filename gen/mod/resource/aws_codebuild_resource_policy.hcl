resource "aws_codebuild_resource_policy" "aws_codebuild_resource_policy" {
  policy       = var.policy
...
output "id" {
  ...
  value       = aws_codebuild_resource_policy.aws_codebuild_resource_policy.id
}
output "provider_region" {
  description = "Region where the provider should be executed."
  type        = string
...
provider "aws" {
  ...
  default_tags {
    tags {
      ...
      "kind/name"                   = "aws_codebuild_resource_policy"
      "kind/version"                = "v0.1.0"
    }
  }
}
