resource "aws_cloudwatch_log_destination" "aws_cloudwatch_log_destination" {
  name       = var.name
...
output "arn" {
  ...
  value       = aws_cloudwatch_log_destination.aws_cloudwatch_log_destination.arn
}
output "provider_region" {
  description = "Region where the provider should be executed."
  type        = string
...
provider "aws" {
  ...
  default_tags {
    tags {
      ...
      "kind/name"                   = "aws_cloudwatch_log_destination"
      "kind/version"                = "v0.1.0"
    }
  }
}
